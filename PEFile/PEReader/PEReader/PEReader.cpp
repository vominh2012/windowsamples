// PEReader.cpp : Defines the entry point for the console application.
//
#include<stdio.h> 
#include<windows.h>
#include<time.h>
...
int main(int argc , char ** argv){
	...
	if(argc>1){
		...
		if(ntHeader->Signature == IMAGE_NT_SIGNATURE){
			...
			printf("\n%-36s%s","Time Stamp :",ctime((time_t *)&(header.TimeDateStamp)));			//Determine Time Stamp
			...
			for(pSecHeader = IMAGE_FIRST_SECTION(ntHeader),i=0;i<ntHeader->FileHeader.NumberOfSections;i++,pSecHeader++){	
				...
				if(argc==4&& (!_strcmpi(argv[2],"-h")||!_strcmpi(argv[2],"/h"))){
					if(!_strcmpi(argv[3],(char *)pSecHeader->Name))
